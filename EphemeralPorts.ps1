...
If ($iCount -gt 0)
{
    ...
    ForEach ($oItem in $overtenpercent)
    {
		Write-Output "WARNING: TCP ephemeral port usage is $($oItem.Percent)% on $($oItem.LocalAddress), review the log file in $OutputFilePath."
